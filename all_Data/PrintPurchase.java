...
public class PrintPurchase implements Printable {
	...
	private void initialize() {
		...
		OutputTable.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "Shop Copy" }));
		...
		btnPrint.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				job.setPrintable(PrintPurchase.this);
				job.validatePage(pf);
	...
	private void fill_data() {
		...
		for (int i = 0; i < CustomerPurchase.custTable.getRowCount()-2; i++) {
			if(isFloat((String) CustomerPurchase.custTable.getValueAt(i, 7)) != 0) {
			String s1 = (String) CustomerPurchase.custTable.getValueAt(i, 1);
			String s2 = (String) CustomerPurchase.custTable.getValueAt(i, 3);
			String s3 = (String) CustomerPurchase.custTable.getValueAt(i, 7);
			String s4 = (String) CustomerPurchase.custTable.getValueAt(i, 6);
			String StockCopy = null;
			StockCopy = s1+" "+s2+"*"+s4+" "+"="+" "+ s3;
			model.addRow(new Object[] { StockCopy });
			}
		}
		String netTotal = "*"+CustomerPurchase.custTable.getValueAt(CustomerPurchase.custTable.getRowCount()-1, 5)+" = "+CustomerPurchase.custTable.getValueAt(CustomerPurchase.custTable.getRowCount()-1, 7);
		model.addRow(new Object[] {netTotal});
	}
	...
	public int print(Graphics g, PageFormat pageFormat, int pageIndex) throws PrinterException {
		...
		g2.drawString("------HM Shop------", 10, 12);
		g2.setFont(new Font("Segoe UI Light", Font.BOLD, 9));
		g2.drawString("Bill Generated on: " + date, 10, 26);
		g2.setFont(new Font("Segoe UI Light", Font.PLAIN, 14));
		g2.drawString("  -------------------", 10, 35);
		...
		for (int i = 0; i < OutputTable.getRowCount(); i++) {
			...
			String Copy = null;
			Copy = s0;
			if(i==OutputTable.getRowCount()-1) {
				g2.setFont(new Font("Segoe UI Light", Font.PLAIN, 14));
				g2.drawString("  -------------------", 10, (35 + spaceIterator++ * 12));
				g2.setFont(new Font("SutonnyMJ", Font.PLAIN, 12));
			}
