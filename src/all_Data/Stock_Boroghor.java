...
public class Stock_Boroghor implements Runnable{
	...
	public static JFrame frmStockSheetBoroghor;
	...
	private boolean isWindowOverlayedStockBoroghor = false;
	
//	private int numberOfRows = 88; 
	private int numberOfRows = MainWindow.filesBoroghor.size(); //Dynamic Loading. //Changed on: 2022-06-21.
	
	public Stock_Boroghor() {
		initialize();
	}
	...
	void setLook() {
		...
		frmStockSheetBoroghor = new JFrame();
		frmStockSheetBoroghor.setAutoRequestFocus(false);
		frmStockSheetBoroghor.setTitle("Boroghor Stock Page ");
		frmStockSheetBoroghor.setBounds(100, 100, 1260, 720);
		frmStockSheetBoroghor.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frmStockSheetBoroghor.setExtendedState(frmStockSheetBoroghor.getExtendedState() | JFrame.MAXIMIZED_BOTH);
		frmStockSheetBoroghor.getContentPane().setLayout(new BorderLayout(0, 0));
		
		frmStockSheetBoroghor.addWindowFocusListener(new WindowFocusListener() {
		...
		frmStockSheetBoroghor.getContentPane().add(Operation_panel, BorderLayout.NORTH);
		Operation_panel.setLayout(new BoxLayout(Operation_panel, BoxLayout.X_AXIS));

		lblDelivery = new JLabel("  Boroghor  Stock Page  ");
		...
		btnGenerateTable.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				new Print_Stock("stock_boroghor");
				
				//New Updated
				isWindowOverlayedStockBoroghor = false;
			}
		});
		Operation_panel.add(btnGenerateTable);
		
		btnSaveTable = new JButton("Save Entry ");
		...
		btnSaveTable.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(frmStockSheetBoroghor, "Saving Successful!!");
				MainWindow.isSaved=true;
		...
		frmStockSheetBoroghor.getContentPane().add(Action_panel, BorderLayout.WEST);
		...
		btnStockEntryBoroghor.setBackground(Color.white);
	...
	public void frame_windowStateChanged_HasFocus(WindowEvent e) {
		if(frmStockSheetBoroghor.isFocused() && (!MainWindow.modifiedRowVectorSalesBoroghor.isEmpty() || !isWindowOverlayedStockBoroghor)) {
			Thread object = new Thread(this); 
	        object.start();
//	        System.out.println("Stock Sheet Focused");
		}	
	}
	
	public void frame_windowStateChanged_LostFocus(WindowEvent e) {
		if(!MainWindow.modifiedRowVectorSalesBoroghor.isEmpty()) {
			isWindowOverlayedStockBoroghor = false;
		}
		else
		isWindowOverlayedStockBoroghor = true;
	}
...
//						StockHistory.removeCellData("Boroghor",MainWindow.filesBoroghor[tcl.getRow()],lastBalance);
						StockHistory.removeCellData("Boroghor",MainWindow.filesBoroghor.get(tcl.getRow()),lastBalance); //Changed on: 2023-06-14.
...
//				System.out.println("Problem Occured");
			...
			MainWindow.modifiedCellVectorStockBoroghor.add(tcl.getRow());
			MainWindow.modifiedRowVectorStockBoroghor.add(tcl.getRow());
		}
	...
	public void fn_render_panel() {
		...
		frmStockSheetBoroghor.getContentPane().add(Table_panel, BorderLayout.CENTER);
	...
	private void fn_fill_data() {
		...
		file = new File("D:\\business_all_stock\\Boroghor\\Boroghor_main_stock_sheet.xls");
		workbook = null;
		try {
			try {
...
//			File tempFile=new File("D:\\business_all_stock\\Boroghor\\"+MainWindow.filesBoroghor[row]+".xls");
			File tempFile=new File("D:\\business_all_stock\\Boroghor\\"+MainWindow.filesBoroghor.get(row)+".xls"); //Changed on: 2023-06-14
...
//				File tempFile = new File("D:\\business_all_stock\\Boroghor\\"+MainWindow.filesBoroghor[index]+".xls");
				File tempFile = new File("D:\\business_all_stock\\Boroghor\\"+MainWindow.filesBoroghor.get(index)+".xls"); //Changed on: 2023-06-14
				FileInputStream fsIP = new FileInputStream(tempFile);
				HSSFWorkbook wb = new HSSFWorkbook(fsIP);
				HSSFSheet worksheet = wb.getSheetAt(0);
	...
	private void fn_RefreshTable() {
			...
			Arrays.fill(MainWindow.updatedRow, 0);
			frmStockSheetBoroghor.setEnabled(false);
			
			//ready
			Iterator<Integer> it = MainWindow.modifiedCellVectorStockBoroghor.iterator();
			while(it.hasNext()) {
				int j=it.next();
//				if(MainWindow.filesBoroghor[j]=="blank") {
//					table.setValueAt("", j, 3);
//				}
				if(MainWindow.filesBoroghor.get(j)=="blank") { //Changed on: 2023-06-14
					table.setValueAt("", j, 3);
				}
				else
				{
			...
			MainWindow.modifiedCellVectorStockBoroghor.clear();
			
			//done
			frmStockSheetBoroghor.setEnabled(true);	
		}
		...
		public void run() {
			...
			Iterator<Integer> i = MainWindow.modifiedRowVectorSalesBoroghor.iterator();
			while(i.hasNext()) {
				int j = i.next();
			...
			isWindowOverlayedStockBoroghor = false;
			MainWindow.modifiedRowVectorSalesBoroghor.clear();
		}
